...
function canQueenCaptureKing(queen, king) {
  const xDiff = Math.abs(queen.x - king.x);
  const yDiff = Math.abs(queen.y - king.y);
...
function getSpiralMatrix(size) {
  ...
  while (startIndex <= endIndex) {
    for (let column = startIndex; column <= endIndex; column += 1) {
      MATRIX[startIndex][column] = currentValue;
      currentValue += 1;
    }

    for (let row = startIndex + 1; row <= endIndex; row += 1) {
      MATRIX[row][endIndex] = currentValue;
      currentValue += 1;
    }

    for (let column = endIndex - 1; column >= startIndex; column -= 1) {
      MATRIX[endIndex][column] = currentValue;
      currentValue += 1;
    }

    for (let row = endIndex - 1; row > startIndex; row -= 1) {
      MATRIX[row][startIndex] = currentValue;
      currentValue += 1;
    }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < MATRIX_LENGTH; i += 1) {
    let temp = [];
    for (let j = MATRIX_LENGTH - 1; j >= 0; j -= 1) {
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 1; i <= iterations; i += 1) {
    ...
    for (let j = 0; j < newString.length; j += 1) {
      if (j % 2 === 0) {
        evenIndexes += newString.charAt(j);
      } else {
        oddIndexes += newString.charAt(j);
      }
    }
    newString = evenIndexes + oddIndexes;
    if (str === newString) {
