...
function dateToTimestamp(date) {
  const baseTime = new Date('01 Jan 1970 00:00:00 UTC');
  const targetTime = new Date(date);
  return targetTime - baseTime;
}
...
function getTime(date) {
  ...
  const hours = timeToString(date.getHours());
  const minutes = timeToString(date.getMinutes());
  const seconds = timeToString(date.getSeconds());
...
function getNextFriday(date) {
  ...
  if (currentWeekDay === 4) {
    ...
    dayToFriday = 5 - currentWeekDay;
  } else if (currentWeekDay > 4) {
    dayToFriday = 5 + 7 - currentWeekDay;
  }

  return new Date(
    Date.UTC(currentYear, currentMonth, currentDate + dayToFriday)
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (
    Math.floor((Date.parse(dateEnd) - Date.parse(dateStart)) / secondsPerDay) +
    1
  );
}
...
function formatDate(date) {
  ...
  const minutes = timeToString(targetDate.getUTCMinutes());
  const seconds = timeToString(targetDate.getUTCSeconds());

  return `${targetDate.getUTCMonth() + 1}/${targetDate.getUTCDate()}/${targetDate.getUTCFullYear()}, ${formattedHours}:${minutes}:${seconds} ${prefix}`;
}
...
function getWeekNumberByDate(date) {
  ...
  let weekNumber = Math.ceil((date.getTime() - firstDay.getTime()) / dayMs / 7);

  if (firstDay.getDay() === date.getDay()) {
    weekNumber += 1;
  }
...
function isLeapYear(date) {
  const targetDate = new Date(date.getFullYear(), 2, 0);
  return targetDate.getDate() === 29;
}
