...
function encodeToRot13(str) {
  const BASE_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const ROT_ALPHABET = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].match(/\W/)) {
      result += str[i];
    } else {
      for (let j = 0; j < BASE_ALPHABET.length; j += 1) {
        if (str[i] === BASE_ALPHABET[j]) {
          result += ROT_ALPHABET[j];
        }
      }
    }
  }
